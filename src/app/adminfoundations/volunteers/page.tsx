...
import { getVolunteers, deleteVolunteer } from "./actions";
...
export default function AdminVolunteersPage() {
  ...
  async function loadVolunteers() {
    try {
      ...
      console.error("Error fetching volunteers:", err.message || err);
  ...
  const filteredVolunteers = volunteers.filter((v) => {
    ...
    return matchesSearch && matchesType && matchesBloodDonor;
  });
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-1">
              View and manage registered individual volunteers, NGOs, and institutions.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3">
            <div className="rounded-2xl border border-slate-200 bg-white px-4 py-2.5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
              <span className="block text-[10px] font-bold uppercase text-slate-400">Total Volunteers</span>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white px-4 py-2.5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
              ...
              <span className="text-lg font-extrabold text-rose-600 dark:text-rose-400">
                {volunteers.filter(v => v.active_blood_donor === 'Yes').length}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-4 sm:p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900 mb-6 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
            ...
            {/* Filter by Blood Donor */}
            <div className="md:col-span-3">
              <select
                value={filterBloodDonor}
                onChange={(e) => setFilterBloodDonor(e.target.value)}
                className="w-full rounded-2xl border border-slate-200 bg-slate-50 py-2.5 px-4 text-xs font-semibold text-slate-700 focus:border-[#798321] focus:outline-none dark:border-zinc-800 dark:bg-zinc-800 dark:text-zinc-200"
              >
                <option value="All">Blood Donors: All</option>
                <option value="Yes">Active Blood Donor: Yes</option>
                <option value="No">Active Blood Donor: No</option>
              </select>
            </div>
          ...
          {loading ? (
            ...
            <div className="py-20 text-center space-y-3">
              ...
              <p className="text-xs font-bold text-slate-500 dark:text-zinc-400">
                {volunteers.length === 0 ? "No volunteers found in the database table." : "No volunteers match your current search/filter criteria."}
              </p>
            </div>
          ) : (
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-slate-100 bg-slate-50/75 text-[11px] font-bold uppercase tracking-wider text-slate-400 dark:border-zinc-800 dark:bg-zinc-900/75">
                    ...
                    <th className="py-4 px-4">Blood Donor</th>
                    ...
                    <tr key={volunteer.id} className="hover:bg-slate-50/50 dark:hover:bg-zinc-800/40 transition">
                      ...
                      <td className="py-4 px-4">
                        {volunteer.blood_group ? (
                          <span className="inline-flex items-center gap-1 font-extrabold text-rose-600 dark:text-rose-400 bg-rose-50 dark:bg-rose-950/40 px-2.5 py-1 rounded-lg border border-rose-200/50 dark:border-rose-900/40">
                            <Droplet size={12} /> {volunteer.blood_group}
                          </span>
                        ) : (
                      ...
                      <td className="py-4 px-4">
                        {volunteer.active_blood_donor === "Yes" ? (
                          <span className="inline-flex items-center gap-1 text-[10px] font-bold text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/40 px-2.5 py-1 rounded-full">
                            Yes
                          </span>
                        ) : (
              ...
              <div className="grid grid-cols-2 gap-4 text-xs">
                <div className="space-y-1">
                  ...
                  <div className="font-semibold text-slate-800 dark:text-zinc-200 flex items-center gap-1.5 truncate">
                    <Mail size={13} className="text-[#798321]" /> {selectedVolunteer.email}
                  </div>
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <div className="font-semibold text-slate-800 dark:text-zinc-200 flex items-center gap-1.5">
                    <Phone size={13} className="text-[#798321]" /> +91 {selectedVolunteer.phone}
                  </div>
                </div>

                <div className="space-y-1">
                  <span className="text-slate-400 block font-bold uppercase text-[10px]">Gender & DOB</span>
                  <div className="font-semibold text-slate-800 dark:text-zinc-200 flex items-center gap-1.5">
                    <Calendar size={13} className="text-[#798321]" /> {selectedVolunteer.gender}, {selectedVolunteer.dob}
                  </div>
                </div>

                <div className="space-y-1">
                  <span className="text-slate-400 block font-bold uppercase text-[10px]">Blood Group</span>
                  <div className="font-semibold text-rose-600 dark:text-rose-400 flex items-center gap-1.5">
                    <Droplet size={13} /> {selectedVolunteer.blood_group || 'Not specified'}
                  </div>
                </div>

                <div className="col-span-2 space-y-1">
                  <span className="text-slate-400 block font-bold uppercase text-[10px]">Address</span>
                  <div className="font-semibold text-slate-800 dark:text-zinc-200 flex items-center gap-1.5">
                    <MapPin size={13} className="text-[#798321] shrink-0" /> {selectedVolunteer.street_address}, {selectedVolunteer.city}
                  </div>
                </div>

                <div className="space-y-1">
                  <span className="text-slate-400 block font-bold uppercase text-[10px]">Active Blood Donor</span>
                  <div className="font-semibold text-slate-800 dark:text-zinc-200">
                    {selectedVolunteer.active_blood_donor}
                  </div>
                </div>

                <div className="space-y-1">
                  <span className="text-slate-400 block font-bold uppercase text-[10px]">Registration Date</span>
                  <div className="font-semibold text-slate-800 dark:text-zinc-200">
