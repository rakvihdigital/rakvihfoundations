...
export default function EditMaterialModal({
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      syllabus_id: String(material.syllabus_id),
      topic_id: String(material.topic_id),
      title: material.title || "",
      description: material.description || "",
      status: material.status || "Published",
    });

    loadModules(String(material.program_id));
    loadTopics(String(material.syllabus_id));
  }, [material]);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      ...
      >
        ...
        <div className="flex items-center justify-between px-8 py-6 border-b border-gray-100 dark:border-slate-700">
          <div>
            <h1 className="text-xl font-bold text-[#24310F] dark:text-white">Edit Material</h1>
            <p className="text-xs text-gray-500 mt-1">Update study material details.</p>
          </div>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <label className="border border-gray-300 rounded-3xl py-10 px-6 flex flex-col items-center cursor-pointer hover:bg-gray-50 transition min-h-[260px]">
                  <Upload size={38} className="text-[#6B7328]" />
                  <p className="mt-4 text-sm font-medium text-[#24310F]">Choose New PDF</p>
                  <p className="text-[10px] text-gray-500 mt-1">Only PDF files allowed</p>
  ...
  async function handleSubmit() {
    try {
      ...
      if (!res.ok || !data.success) {
        alert(data.message || "Unable to update material.");
        return;
      }
