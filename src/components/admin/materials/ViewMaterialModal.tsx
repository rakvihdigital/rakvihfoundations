      ...
      <div className="w-full max-w-5xl max-h-[92vh] overflow-hidden rounded-3xl bg-white dark:bg-[#0B1C33] shadow-2xl flex flex-col">
        ...
        <div className="bg-gradient-to-r from-[#6B7328] to-[#FFC107] px-6 py-5 flex items-center justify-between flex-shrink-0">
          <div>
            ...
            <p className="text-white/90 text-xs mt-1">Complete study material information</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
          ...
          {/* First Row: Very Small Image + Two Details */}
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            
            {/* Very Small Image */}
            <div className="lg:col-span-3">
              {material.thumbnail ? (
                <div className="overflow-hidden rounded-3xl border border-gray-200 dark:border-slate-700 shadow-md">
                  <img
                    src={material.thumbnail}
                    alt={material.title}
                    className="w-full h-40 object-cover"   // ← Very Small Size
                  />
                </div>
              ) : (
                <div className="h-40 bg-gray-100 dark:bg-[#1A2A44] rounded-3xl flex items-center justify-center border border-dashed border-gray-300">
                  <FileText size={38} className="text-gray-400" />
                </div>
              )}
            </div>

            {/* Two Details beside Image (Height matched) */}
            <div className="lg:col-span-9 grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <InfoCard
                ...
                label="Program"
                value={material.programs?.title}
              />
            </div>
          </div>

          {/* Second Row: 3 Details */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <InfoCard
              icon={<Layers size={17} />}
              label="Module"
              value={material.syllabus?.module_name}
            />
            <InfoCard
              icon={<FolderOpen size={17} />}
              label="Topic"
              value={material.topics?.topic}
            />
            <InfoCard
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <InfoCard
              ...
              label="Thumbnail"
              value={material.thumbnail ? "Available" : "Not Uploaded"}
            />
            <InfoCard
            ...
            <InfoCard
              ...
              label="Downloads"
              value={material.downloads ?? 0}
            />
            <InfoCard
              icon={<Calendar size={17} />}
              label="Created On"
              value={material.created_at ? new Date(material.created_at).toLocaleDateString("en-IN") : "-"}
            />
            <InfoCard
              icon={<Calendar size={17} />}
              label="Updated On"
              value={material.updated_at ? new Date(material.updated_at).toLocaleDateString("en-IN") : "-"}
            />
          </div>
          ...
          <div className="rounded-2xl border border-[#E8ECE5] dark:border-slate-700 bg-[#F8FAF3] dark:bg-[#081525] p-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 text-sm">
              <div>
                ...
                <p className="mt-1 font-semibold text-gray-900 dark:text-white break-all">{material.file_name || "-"}</p>
              </div>
              <div>
                <p className="text-xs uppercase tracking-widest text-gray-500">File Size</p>
                <p className="mt-1 font-semibold text-gray-900 dark:text-white">{material.file_size || "-"}</p>
              </div>
              <div>
                <p className="text-xs uppercase tracking-widest text-gray-500">Type</p>
                <p className="mt-1 font-semibold text-gray-900 dark:text-white">{material.file_type || "-"}</p>
              </div>
              <div>
                <p className="text-xs uppercase tracking-widest text-gray-500">Downloads</p>
                <p className="mt-1 font-semibold text-gray-900 dark:text-white">{material.downloads ?? 0}</p>
              </div>
            </div>
          </div>
