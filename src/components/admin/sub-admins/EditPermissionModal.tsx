...
export default function EditPermissionModal({
  ...
  useEffect(() => {
    ...
    setPermissions(
      admin.admin_permissions?.[0] || defaultPermissions
    );
  }, [admin]);
  ...
  async function updateAdmin() {
    try {
      ...
      const res = await fetch(`/api/admin/sub-admins/${admin.id}`, {
        method: "PUT",
        headers: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-2xl bg-white dark:bg-gray-900 p-6 shadow-2xl scrollbar-thin scrollbar-thumb-[#6B7328] scrollbar-track-gray-100">
