...
export default function SuccessStoriesSection() {
  return (
    <section className="relative overflow-hidden py-12 sm:py-16 lg:py-20 bg-[#FDFDFB] dark:bg-black transition-colors duration-500">
      {/* Background Blurs */}
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6">
        ...
        >
          ...
          <p className="mt-3 max-w-2xl mx-auto text-xs md:text-sm text-slate-600 leading-relaxed dark:text-neutral-400">
            Every RAKVIH success story reflects the same commitment: quality live-project learning, genuine industry exposure, and a structured path to career transformation. These are outcomes you can verify — not marketing claims.
          </p>
        </motion.div>
        ...
        <div className="grid lg:grid-cols-[1.2fr,0.8fr] gap-10 items-center">
          ...
          <motion.div
            ...
            transition={{ duration: 0.6 }}
            viewport={{ once: true }}
            className="relative h-[280px] sm:h-[360px] lg:h-[500px] rounded-[30px] overflow-hidden shadow-2xl border-[6px] border-[#798321] ring-4 ring-[#FFC107]/30 p-1.5"
          >
            ...
            {/* Bottom Glass Card */}
            <div className="absolute bottom-4 left-4 right-4 sm:bottom-6 sm:left-6 sm:right-6 rounded-2xl border border-white/20 bg-white/10 backdrop-blur-md p-3 sm:p-4">
              <div className="grid grid-cols-3 gap-2">
                {stats.map((item) => (
                  <div key={item.label} className="text-center">
                    <h3 className="text-lg sm:text-xl font-black text-[#FFC107]">{item.number}</h3>
                    <p className="text-[7px] sm:text-[8px] uppercase tracking-[0.1em] text-white mt-1">{item.label}</p>
                  </div>
                ))}
              </div>
            </div>
         ...
         <motion.div
          ...
          >
            ...
            <div className="mb-8">
              ...
              >
                The same five pillars behind every program on our Programs page.
              </motion.p>
            </div>
